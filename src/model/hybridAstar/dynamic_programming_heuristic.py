...
def calc_distance_heuristic(gx, gy, ox, oy, resolution, rr):
    ...
    # Convert obstacle coordinates from real-world units (meters) to grid coordinates.
    ...
    # open_set: nodes to be explored, closed_set: visited nodes
    ...
    for ix in range(x_width):
        ...
        for iy in range(y_width):
            ...
            for iox, ioy in zip(ox, oy):
                ...
                # the distance between grid center to obstacle
                d = math.hypot(iox - x, ioy - y)
                # inflated map
                # Mark the cell as occupied if it lies within the robot's radius from any
                # obstacle point. This performs obstacle inflation in configuration space,
                # ensuring that the robot (treated as a circle with radius vr) does not
                # collide with obstacles during planning.
                if d <= vr / resolution:
                    obstacle_map[ix][iy] = True
                    break

    # obstacle_map: 2D boolean array where True indicates obstacle, False indicates free space
...
def get_motion_model():
    # 8-connected movement model
    # Cost for straight moves: 1 (distance = 1 grid cell)
    # Cost for diagonal moves: sqrt(2) (distance = sqrt(1² + 1²) grid cells)
    # dx, dy, cost
    motion = [[1, 0, 1],
              [0, 1, 1],
              [-1, 0, 1],
